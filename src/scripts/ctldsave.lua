...
function loaddata()
  ...
  if file_exists(savefile) then --Script has been run before, so we need to load the save
	...
	if ctldsave[1] ~= nil then
		BASE:E({"CTLD LOAD AAsystems",ctld.completeAASystems,ctldsave[1]})
		ctld.completeAASystems = ctldsave[1]
	else
		BASE:E({"CTLD LOAD AAsystems nil",ctld.completeAASystems,ctldsave[1]})
...
function savectldpersistence()
	if dataloaded == true then
		...
		BASE:E({"Attempting to save CTLD Data with out Dataload being true"})
	end
end
...
if init == true then
  if dataloaded == false then
	BASE:E({"DATALOADED was FALSE IS NOW loading and should be true",dataloaded})
	loaddata()
  else 
	BASE:E({"DATALOADED was Not FALSE IS NOW loading and should be true"})
	savectldpersistence()
  end
